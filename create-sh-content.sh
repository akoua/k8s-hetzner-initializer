...
if [ -z "${create_snapshots}" ] ; then
    echo " "
    echo "The snapshots are required and deployed using packer. If you need specific extra packages, you need to choose no and edit hcloud-image-snapshots.pkr.hcl file manually. This is not needed in 99% of cases, as we already include the most common packages."
    echo " "
...
if [[ "$create_snapshots" =~ ^([Yy]es|[Yy])$ ]]; then
    ...
    echo "Running packer build for hcloud-image-snapshots.pkr.hcl"
    cd "${folder_path}" && packer init hcloud-image-snapshots.pkr.hcl && packer build hcloud-image-snapshots.pkr.hcl
else
    echo " "
    echo "You can create the snapshots later by running 'packer build hcloud-image-snapshots.pkr.hcl' in the folder."
fi
